...
const dataColumnLengths = []
const labelColumnLengths = []
let dataStartRow
...
const labelStopRow = dataStartRow - 1
...
Object.keys(metric_data.values)
    ...
    .forEach((key) => {
        ...
        let subKeys = key.split('-') // key is the string with breakouts, for example,  "client-2-10" for <cstype>-<csid>-<num> for source: mpstat type: Busy-CPU
        ...
        subKeys.forEach((subKey) => {
            const subMetric = subKey.replace(/<(\w+)>/, '$1')
            i
