...
const subtractTwoArrays = function (a1, a2) {
    const a3 = []
    a1.forEach((element) => {
...
const intersectTwoArrays = function (a1, a2) {
    const a3 = []
    a1.forEach((element) => {
        if (!a2.includes(element)) {
            return
...
const intersectAllArrays = function (a2D) {
    let intersectArray = a2D[0]
    a2D.forEach((a1D) => {
...
const mSearch = function (url, index,termKeys,values,source,aggs,size,sort
    ...
    for (var i = 0; i < values[0].length; i++) {
        ...
        for (let x = 0; x < termKeys.length; x++) {
            const termStr =
                '{ "term": { "' + termKeys[x] + '": "' + values[x][i] + '"}}'
    ...
    const resp = esRequest(url, index + '/_doc/_msearch', ndjson)
    const data = JSON.parse(resp.getBody())
    ...
    for (var i = 0; i < data.responses.length; i++) {
        ...
        if (
            ...
            Array.isArray(data.responses[i].hits.hits) &&
            data.responses[i].hits.hits.length > 0
        ) {
            if (
                ...
                if (source !== '' && source !== null) {
                    ...
                    source.split('.').forEach((thisObj) => {
                        if (typeof obj[thisObj] === 'undefined') {
                            console.log(
                                'WARNING: the requested source for this query [' +
                                    source +
                                    '] does not exist in the returned data:\n'
                            )
...
mgetSampleStatus = function (url, Ids) {
    ...
    const sampleStatus = [] // Will be 2D array of [iter][sampIds];
    idx = 0
    for (var i = 0; i < Ids.length; i++) {
        for (j = 0; j < Ids[i].length; j++) {
...
mgetPrimaryPeriodId = function (url, sampIds, periNames) {
    ...
    const periodIds = [] // Will be 2D array of [iter][periIds];
    idx = 0
    for (var i = 0; i < sampIds.length; i++) {
        for (j = 0; j < sampIds[i].length; j++) {
...
mgetPeriodRange = function (url, periodIds) {
    ...
    const data = mSearch(url, 'period', ['period.id'], [Ids], 'period', null, 1)
    // console.log("data:\n" + JSON.stringify(data, null, 2));
    // mSearch returns a 2D array, in other words, a list of values (inner array) for each query (outer array)
    // In this case, the queries are 1 per sampleId/periodName (for all iterations ordered), and the list of values
    // happens to be exactly 1 value, the primaryPeriodId.
    const ranges = [] // Will be 2D array of [iter][periIds];
    idx = 0
    for (var i = 0; i < periodIds.length; i++) {
...
mgetIterMetrics = function (url, iterationIds) {
    const results = {}
    const benchmarkNames = consolidateAllArrays(
        mgetBenchmarkNameFromIter(url, iterationIds)
    )
    if (benchmarkNames.length !== 1) {
        console.log(
            'ERROR: The benchmark-name for all iterations was not the same, includes: ' +
                benchmarkNames
        )
        process.exit(1)
    }
    const primaryMetrics = consolidateAllArrays(
        mgetPrimaryMetric(url, iterationIds)
    )
    if (primaryMetrics.length !== 1) {
        console.log(
            'ERROR: The primary-metric for all iterations was not the same, includes: ' +
                primaryMetrics
        )
        process.exit(1)
    }
    const primaryPeriodNames = consolidateAllArrays(
        mgetPrimaryPeriodName(url, iterationIds)
    )
    if (primaryPeriodNames.length !== 1) {
        console.log(
            'ERROR: The primary-period-name for all iterations was not the same, includes: ' +
                primaryPeriodNames
        )
        process.exit(1)
    }
    ...
    const samples = mgetSamples(url, iterationIds) // Samples organized in 2D array, first dimension matching iterationIds
    // console.log("\nsamples:\n" + JSON.stringify(samples, null, 2));
    const samplesByIterId = {}
    const iterIdFromSample = {}
    for (i = 0; i < iterationIds.length; i++) {
        var iterId = iterationIds[i]
        const thisIterSamples = samples[i] // Array
    ...
    const consSamples = consolidateAllArrays(samples) // All sample IDs flattened into 1 array
    const consSamplesStatus = mgetSampleStatus(url, consSamples)
    const consPassingSamples = [] // Only passing samples in flattened array
    for (i = 0; i < consSamplesStatus.length; i++) {
        if (consSamplesStatus[i] == 'pass') {
            consPassingSamples.push(consSamples[i])
        }
    }
    // console.log("\nconsPassingSamples: " + consPassingSamples);
    // console.log("mgetPrimaryperiodId");
    const primaryPeriodIds = mgetPrimaryPeriodId(
        url,
        consPassingSamples,
        primaryPeriodNames
    )
    const periodsBySample = {}
    const sampleIdFromPeriod = {}
    for (i = 0; i < consPassingSamples.length; i++) {
        var sampId = consPassingSamples[i]
        const thisSamplePeriods = primaryPeriodIds[i] // Array
        periodsBySample[sampId] = thisSamplePeriods
        thisSamplePeriods.forEach((p) => {
            sampleIdFromPeriod[p] = sampId
        })
    }
    const consPrimaryPeriodIds = consolidateAllArrays(primaryPeriodIds)
    // console.log("\nconsPrimaryPeriodIds: " + JSON.stringify(consPrimaryPeriodIds, null , 2));
    // console.log("mgetPeriodRange");
    const periodRanges = mgetPeriodRange(url, consPrimaryPeriodIds)
    // console.log("\nperiodRanges: " + JSON.stringify(periodRanges, null, 2));
    // Create the sets for getMetricDataSets
    const sets = []
    const periodsByIteration = {}
    ...
    const metricDataSets = getMetricDataSets(url, sets)
    // console.log("calcIterMetrics");
    // console.log("\nMetricDataSets.length: " + metricDataSets.length);
    // Build per-iteration results
    let period = consPrimaryPeriodIds[0]
    let sample = sampleIdFromPeriod[period]
    let iter = iterIdFromSample[sample]
    let vals = []
...
deleteDocs = function (url, docTypes, q) {
    docTypes.forEach((docType) => {
        ...
        const resp = esRequest(url, docType + '/_doc/_delete_by_query', q)
        const data = JSON.parse(resp.getBody())
    })
}
...
deleteMetrics = function (url, runId) {
    const ids = getMetricDescs(url, runId)
    // console.log("There are " + ids.length + " metric_desc docs");
    const q = {
        query: { bool: { filter: { terms: { 'metric_desc.id': [] } } } },
    }
    ids.forEach((element) => {
        const term = { 'metric_desc.id': element }
    ...
    const remaining = q.query.bool.filter.terms['metric_desc.id'].length
    if (remaining > 0) {
        // console.log("deleting " + q['query']['bool']['filter']['terms']["metric_desc.id"].length + " metrics");
        deleteDocs(url, ['metric_data', 'metric_desc'], q)
    }
...
buildIterTree = function (
    ...
    const iterNode = {}
    const newParamsJsonStr = JSON.stringify(params)
    const newParams = JSON.parse(newParamsJsonStr)
    const newTagsJsonStr = JSON.stringify(tags)
    const newTags = JSON.parse(newTagsJsonStr)
    ...
    Object.keys(newParams).forEach((arg) => {
        if (Object.keys(newParams[arg]).length == 1) {
            ...
            const val = Object.keys(newParams[arg])[0] // the one and only value
            const thisParam = { arg, val }
    ...
    Object.keys(newTags).forEach((name) => {
        if (Object.keys(newTags[name]).length == 1) {
            ...
            const val = Object.keys(newTags[name])[0] // the one and only value
            const thisTag = { name, val }
    ...
    const args = Object.keys(newParams).filter(
        (x) => !dontBreakoutParams.includes(x)
    )
    if (args.length > 0) {
        ...
        Object.keys(newParams[nextArg]).forEach((val) => {
            const intersectedIterIds = intersectTwoArrays(
                iterIds,
                newParams[nextArg][val]
            )
            ...
            if (intersectedIterLength == 0) {
                ...
                let newIter
                const newNewParamsJsonStr = JSON.stringify(newParams)
                const newNewParams = JSON.parse(newNewParamsJsonStr)
                ...
                if (
                    typeof newIter !== 'undefined' &&
                    Object.keys(newIter).length > 0
                ) {
                    ...
                    console.log(
                        indent +
                            'warning: newIter undefined or empty:\n' +
    ...
    const names = Object.keys(newTags).filter(
        (x) => !dontBreakoutTags.includes(x)
    )
    if (names.length > 0) {
        ...
        Object.keys(newTags[nextName]).forEach((val) => {
            ...
            if (intersectedIterLength == 0) {
                ...
                intersectedIterCount += intersectedIterLength
                let newIter
                const newNewTagsJsonStr = JSON.stringify(newTags)
                const newNewTags = JSON.parse(newNewTagsJsonStr)
    ...
    iterIds.forEach((id) => {
        ...
        Object.keys(newTags).forEach((name) => {
            if (typeof tagValueByIterAndName[id][name] !== 'undefined') {
                thisIter.labels +=
                    ' ' + name + ':' + tagValueByIterAndName[id][name]
            }
        })
        Object.keys(newParams).forEach((arg) => {
            if (typeof paramValueByIterAndArg[id][arg] !== 'undefined') {
                thisIter.labels +=
                    ' ' + arg + ':' + paramValueByIterAndArg[id][arg]
            }
        })
...
reportIters = function (iterTree, indent, count) {
    ...
    if (typeof iterTree.iterations === 'undefined') {
        ...
        if (
            typeof iterTree.breakout !== 'undefined' &&
            iterTree.breakout.length > 0
        ) {
            iterTree.breakout.forEach((iter) => {
                const retCount = reportIters(iter, '  ' + indent, 0)
...
getIters = function (
    ...
    const indexjson = ''
    const qjson = ''
    const newestDay = now - 1000 * 3600 * 24 * filterByAge.split('-')[0]
    const oldestDay = now - 1000 * 3600 * 24 * filterByAge.split('-')[1]
    ...
    const base_q_json = JSON.stringify(base_q)
    ...
    console.log(
        'Get all iterations from ' + filterByTags.length + ' tag filters'
    )
    filterByTags.forEach((nameval) => {
        const tag_query = JSON.parse(base_q_json)
        const name = nameval.split(':')[0]
        const val = nameval.split(':')[1]
        const tagNameTerm = { term: { 'tag.name': name } }
        tag_query.query.bool.filter.push(tagNameTerm)
        if (val != 'tag-not-used') {
            const tagValTerm = { term: { 'tag.val': val } }
            ...
            data2.responses.forEach((response) => {
                response.hits.hits.forEach((run) => {
                    if (intersectedRunIds.includes(run._source.run.id)) {
                        const index = intersectedRunIds.indexOf(
                            run._source.run.id
                        )
                        if (index != -1) {
    ...
    const iterIdsFromRun = getIterations(url, intersectedRunIds)
    ...
    console.log(
        'Get all iterations from ' + filterByParams.length + ' param filters'
    )
    ndjson = ''
    filterByParams.forEach((argval) => {
        const param_query = JSON.parse(base_q_json)
        const arg = argval.split(':')[0]
        const val = argval.split(':')[1]
        param_query._source = 'iteration.id'
        const paramArg = { term: { 'param.arg': arg } }
        param_query.query.bool.filter.push(paramArg)
        if (val != 'param-not-used') {
            const paramVal = { term: { 'param.val': val } }
            ...
            data2.responses.forEach((response) => {
                response.hits.hits.forEach((hit) => {
                    if (iterIdsFromParam.includes(hit._source.iteration.id)) {
                        const index = iterIdsFromParam.indexOf(
                            hit._source.iteration.id
                        )
                        if (index !== -1) {
    ...
    const allIterIds = allFilterIterIds
    ...
    if (typeof addRuns !== 'undefined' && addRuns != []) {
        ...
        const ids = getIterations(url, addRuns)
        ids.forEach((id) => {
    ...
    const iterRunIds = mgetRunFromIter(url, allIterIds)
    // console.log("runIds from Iters:\n" + JSON.stringify(runIds, null, 2));
    const iterTags = mgetTags(url, iterRunIds)
    const allTagNames = getObjVals(consolidateAllArrays(iterTags), 'name')
    console.log('allTagNames:\n' + JSON.stringify(allTagNames, null, 2))
    console.log('Finding all param args')
    const iterParams = mgetParams(url, allIterIds)
    const allParamArgs = getObjVals(consolidateAllArrays(iterParams), 'arg')
    ...
    const paramValueByIterAndArg = {}
    const tagValueByIterAndName = {}
    const iterations = []
    ...
    for (j = 0; j < allIterIds.length; j++) {
        ...
        for (var i = 0; i < l; i++) {
            ...
            if (typeof paramIdx[arg] !== 'undefined') {
                ...
                const existing_arg_idx = paramIdx[arg]
        ...
        var tags = iterTags[j]
        const thisIter = { iterId: iter, tags, params }
        var loggedParams = []
    ...
    const notCommonTagNames = []
    const notCommonParamArgs = []
    for (j = 0; j < allIterIds.length; j++) {
        var iter = allIterIds[j]
    ...
    const commonTagNames = [...allTagNames]
    const commonParamArgs = [...allParamArgs]
    ...
    notCommonTagNames.forEach((name) => {
        ...
        for (let i = 0; i < iterations.length; i++) {
            const iterId = iterations[i].iterId
            let foundTag = false
            for (let j = 0; j < iterations[i].tags.length; j++) {
            ...
            if (foundTag == false) {
                const newTag = { name, val: 'tag-not-used' }
    ...
    notCommonParamArgs.forEach((arg) => {
        for (let i = 0; i < iterations.length; i++) {
            ...
            for (let j = 0; j < iterations[i].params.length; j++) {
            ...
            if (foundParam == false) {
                const newParam = { arg, val: 'param-not-used' }
    ...
    const sortedTagNames = Object.keys(tags).sort((a, b) =>
        a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' })
    )

    sortedTagNames.forEach((name) => {
        const sortedTagValues = Object.keys(tags[name]).sort((a, b) =>
    ...
    const results = mgetIterMetrics(url, allIterIds)
...
exports.getMetricSources = function (url, runId) {
    const q = {
        ...
        aggs: {
            source: {
                terms: { field: 'metric_desc.source', size: bigQuerySize },
            },
        },
        size: 0,
    }
    // console.log("Q:\n" + JSON.stringify(q, null, 2));
    const resp = esRequest(url, 'metric_desc/_doc/_search', q)
    const data = JSON.parse(resp.getBody())
...
exports.getDocCount = function (url, runId, docType) {
    const q = { query: { bool: { filter: [{ term: { 'run.id': runId } }] } } }
    const resp = esRequest(url, docType + '/_doc/_count', q)
    const data = JSON.parse(resp.getBody())
    return data.count
}
...
getMetricGroupTermsFromAgg = function (agg, terms) {
    ...
    let count = 0
    let metricGroupTerms = new Array()
...
getBreakoutAggregation = function (source, type, breakout) {
    let agg_str = '{'
    agg_str +=
        '"metric_desc.source": { "terms": { "field": "metric_desc.source"}'
    agg_str +=
        ',"aggs": { "metric_desc.type": { "terms": { "field": "metric_desc.type"}'
    ...
    metricGroupTerms.forEach((term) => {
        ...
        terms.forEach((thisTerm) => {
            Object.keys(thisTerm.term).forEach((field) => {
                ...
                if (
                    field == 'metric_desc.source' ||
                    field == 'metric_desc.type'
                ) {
                    return
                }
...
mgetMetricIdsFromTerms = function (url, termsSets) {
    ...
    for (i = 0; i < termsSets.length; i++) {
        ...
        Object.keys(termsSets[i].termsByLabel)
            ...
            .forEach((label) => {
                ...
                const terms_string = termsSets[i].termsByLabel[label]
                const q = {
                    query: {
                        bool: { filter: JSON.parse('[' + terms_string + ']') },
                    },
                ...
                if (periId == null && runId == null) {
                    console.log(
                        'ERROR: mgetMetricIdsFromTerms(), terms[' +
                            i +
                            ']  must have either a period-id or run-id\n'
                    )
                    return
                }
                if (periId != null) {
                    q.query.bool.filter.push(
                        JSON.parse('{"term": {"period.id": "' + periId + '"}}')
                    )
                }
                if (runId != null) {
                    q.query.bool.filter.push(
                        JSON.parse('{"term": {"run.id": "' + runId + '"}}')
                    )
                }
                ndjson += '{}\n' + JSON.stringify(q) + '\n'
                totalReqs++
    ...
    const resp = esRequest(url, 'metric_desc/_doc/_msearch', ndjson)
    const data = JSON.parse(resp.getBody())
    if (totalReqs != data.responses.length) {
        console.log(
            'mgetMetricIdsFromTerms(): ERROR, number of _msearch responses did not match number of requests'
        )
        return
    }
    ...
    const metricIdsSets = [] // eventual length = termsSets
    let count = 0
    for (i = 0; i < termsSets.length; i++) {
        ...
        Object.keys(termsSets[i].termsByLabel)
            ...
            .forEach((label) => {
                ...
                if (data.responses[i] == null) {
                    ...
                    console.log(
                        'data.responses.length:' + data.responses.length
                    )
                    console.log(
                        'data.responses:\n' +
                ...
                if (
                    data.responses[i].hits.total.value >= bigQuerySize ||
                    data.responses[i].hits.hits.length >= bigQuerySize
                ) {
                    console.log(
                        'ERROR: hits from returned query exceeded max size of ' +
                            bigQuerySize
                    )
                    process.exit(1)
                }
                ...
                for (j = 0; j < data.responses[count].hits.hits.length; j++) {
                    ...
                    thisMetricIds[label].push(
                        data.responses[count].hits.hits[j]._source.metric_desc
                            .id
                    )
                }
                count++
...
getMetricGroupsFromBreakouts = function (url, sets) {
    const metricGroupIdsByLabel = []
    const indexjson = '{}\n'
    const index = JSON.parse(indexjson)
    let ndjson = ''

    sets.forEach((set) => {
        const result = getBreakoutAggregation(
            set.source,
            set.type,
            set.breakout
        )
        const aggs = JSON.parse(result)
        const q = {
            query: {
                bool: {
                    filter: [
        ...
        if (set.period != null) {
            q.query.bool.filter.push(
                JSON.parse('{"term": {"period.id": "' + set.period + '"}}')
            )
        }
        if (set.run != null) {
            q.query.bool.filter.push(
                JSON.parse('{"term": {"run.id": "' + set.run + '"}}')
            )
        }
    ...
    let metricGroupIdsByLabelSets = []
    const metricGroupTermsSets = []
    const metricGroupTermsByLabelSets = []
    const termsSets = []
    for (let idx = 0; idx < sets.length; idx++) {
        ...
        const metricGroupTerms = getMetricGroupTermsFromAgg(
            data.responses[idx].aggregations
        )
        // Derive the label from each group and organize into a dict, key = label, value = the filter terms
        const metricGroupTermsByLabel =
            getMetricGroupTermsByLabel(metricGroupTerms)
        const thisLabelSet = {
            run: sets[idx].run,
...
getMetricGroupsFromBreakout = function (
    url,
    runId,
    periId,
    source,
    ...
    const thisSet = { run: runId, period: periId, source, type, breakout }
    const metricGroupIdsByLabelSets = getMetricGroupsFromBreakouts(url, [
        thisSet,
    ])
...
getMetricDataFromIdsSets = function (url, sets, metricGroupIdsByLabelSets) {
    ...
    for (var idx = 0; idx < metricGroupIdsByLabelSets.length; idx++) {
        Object.keys(metricGroupIdsByLabelSets[idx])
            ...
            .forEach(function (label) {
                const metricIds = metricGroupIdsByLabelSets[idx][label]
                if (typeof sets[idx].begin === 'undefined') {
                    console.log(
                        'ERROR: sets.[' +
                            idx +
                            '].begin is not defined:\n' +
                            JSON.stringify(sets[idx]),
                        null,
                        2
                    )
                    process.exit(1)
                }
                const begin = Number(sets[idx].begin)
                if (isNaN(begin)) {
                ...
                const end = Number(sets[idx].end)
                const resolution = Number(sets[idx].resolution)
    ...
    const resp = esRequest(url, 'metric_data/_doc/_msearch', ndjson)
    const data = JSON.parse(resp.getBody())
    const elements = data.responses.length
    ...
    for (var idx = 0; idx < metricGroupIdsByLabelSets.length; idx++) {
        ...
        Object.keys(metricGroupIdsByLabelSets[idx])
            ...
            .forEach(function (label) {
                ...
                const numMetricIds = metricIds.length
                while (true) {
                    const timeWindowDuration = thisEnd - thisBegin + 1
                    const totalWeightTimesMetrics =
                        timeWindowDuration * numMetricIds
                    subCount++
                    var aggAvg
                    var aggWeight
                    ...
                    if (typeof aggAvg !== 'undefined') {
                        ...
                        // average.
                        aggWeight =
                            data.responses[count + 1].aggregations.total_weight
                                .value
                    ...
                    for (k = 2; k < 4; k++) {
                        ...
                        data.responses[count + k].hits.hits.forEach(
                            (element) => {
                                // for my $key (keys %{ $$j{'_source'}{'metric_data'} }) {
                                partialDocs[element._id] = {}
                                Object.keys(
                                    element._source.metric_data
                                ).forEach((key) => {
                                    // partial_docs[{$$j{'_id'}}{$key} = $$j{'_source'}{'metric_data'}{$key};
                                    partialDocs[element._id][key] =
                                        element._source.metric_data[key]
                                })
                            }
                        )
                    }
                    ...
                    Object.keys(partialDocs).forEach((id) => {
                        let docDuration = partialDocs[id].duration
                        ...
                        const valueTimesWeight =
                            partialDocs[id].value * docDuration
                        sumValueTimesWeight += valueTimesWeight
                        sumWeight += docDuration
                    })
                    let result =
                        (aggAvgTimesWeight + sumValueTimesWeight) /
                        totalWeightTimesMetrics
                    result *= numMetricIds
...
getMetricData = function (
    ...
    sets.push(thisSet)
    const dataSets = getMetricDataSets(url, sets)
    return dataSets[0]
}
...
getMetricDataSets = function (url, sets) {
    for (var i = 0; i < sets.length; i++) {
        ...
        if (typeof sets[i].begin === 'undefined') {
            if (typeof sets[i].period !== 'undefined') {
                ...
                console.log(
                    'ERROR: begin is not defined and a period was not defined'
                )
            }
        }
        if (typeof sets[i].end === 'undefined') {
            if (typeof sets[i].period !== 'undefined') {
                ...
                console.log(
                    'ERROR: end is not defined and a period was not defined'
                )
            }
        }
    ...
    const metricGroupIdsByLabelSets = getMetricGroupsFromBreakouts(url, sets)
    const dataSets = getMetricDataFromIdsSets(
        url,
        sets,
    ...
    const setBreakouts = mgetMetricNames(url, runIds, sources, types)
    ...
    for (var i = 0; i < sets.length; i++) {
        ...
        dataSets[i].valueSeriesLabelDecoder = dataSets[
            i
        ].valueSeriesLabelDecoder.replace('-', '')
        // Breakouts already used should not show up in the list of avauilable breakouts
        dataSets[i].remainingBreakouts = setBreakouts[i].filter(
            (n) => !dataSets[i].usedBreakouts.includes(n)
        )
    }
    ...
    for (var i = 0; i < sets.length; i++) {
        ...
        if (sets[i].filter != null && m) {
            Object.keys(dataSets[i].values).forEach((metric) => {
                const metricValue = 1.0 * dataSets[i].values[metric][0].value
                const condition = m[1]
                const value = m[2]
                if (
                    !(
