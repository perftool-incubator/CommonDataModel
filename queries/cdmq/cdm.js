...
mgetIterMetrics = function (url, iterationIds) {
  var results = {}
  var benchmarkNames = consolidateAllArrays(
  ...
  var primaryMetrics = consolidateAllArrays(
  ...
  var primaryPeriodNames = consolidateAllArrays(
  ...
  var primaryPeriodIds = mgetPrimaryPeriodId(
    url,
    consPassingSamples,
    primaryPeriodNames
  )
...
deleteMetrics = function (url, runId) {
  ...
  ids.forEach((element) => {
    ...
    if (
      q['query']['bool']['filter']['terms']['metric_desc.id'].length >= 1000
    ) {
...
buildIterTree = function (
  ...
  var args = Object.keys(newParams).filter(
      ...
      .forEach((label) => {
        ...
        for (j = 0; j < data.responses[count].hits.hits.length; j++) {
          ...
          thisMetricIds[label].push(
            data.responses[count].hits.hits[j]._source.metric_desc.id
          )
        }
        count++
      })
...
getMetricGroupsFromBreakouts = function (url, sets) {
  ...
  sets.forEach((set) => {
    ...
    set.breakout.forEach((field) => {
      ...
      if (matches) {
        ...
        q.query.bool.filter.push(
          JSON.parse(
            '{"term": {"metric_desc.names.' + field + '": "' + value + '"}}'
          )
        )
      }
    })
  ...
  for (var idx = 0; idx < sets.length; idx++) {
    ...
    var metricGroupTerms = getMetricGroupTermsFromAgg(
    ...
    Object.keys(metricGroupIdsByLabelSets[idx])
      ...
      .forEach(function (label) {
        ...
        while (true) {
          ...
          indexjson =
            '{"index": "' + getIndexBaseName() + 'metric_data' + '" }\n'
          reqjson = '{'
          reqjson += '  "size": 0,'
          reqjson += '  "query": {'
          reqjson += '    "bool": {'
      ...
      .forEach(function (label) {
        ...
        while (true) {
          ...
          if (typeof aggAvg != 'undefined') {
            ...
            // average.
            aggWeight =
              data.responses[count + 1].aggregations.total_weight.value
          ...
          var result =
            (aggAvgTimesWeight + sumValueTimesWeight) / totalWeightTimesMetrics
          result *= numMetricIds
