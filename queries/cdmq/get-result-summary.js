...
runIds.forEach((runId) => {
  ...
  var iterPrimaryPeriodNames = cdm.mgetPrimaryPeriodName(
    program.url,
    benchIterations
  )
  //input: 1D array
  ...
  var iterPrimaryPeriodIds = cdm.mgetPrimaryPeriodId(
    program.url,
    iterSampleIds,
    iterPrimaryPeriodNames
  )
  var iterPrimaryPeriodRanges = cdm.mgetPeriodRange(
    program.url,
    ...
    for (var j = 0; j < iterSampleIds[i].length; j++) {
      ...
      for (var k = 0; k < primaryMetrics.length; k++) {
        ...
        if (sourceType.length == 1) {
          ...
          console.log(
            'sourceType array is an unexpected length, ' + sourceType.length
          )
          process.exit(1)
        }
        var set = {
...
*/
    ...
    for (var j = 0; j < iterSampleIds[i].length; j++) {
      if (
        ...
        logOutput(
          '          primary period-id: ' + iterPrimaryPeriodIds[i][j],
          noHtml
        )
        logOutput(
          '          period range: begin: ' +
